...
const labels = {
  en: {
    ...
    loading: "Loading…",
    error: "Failed to load gazettes.",
  },
  mr: {
    title: "प्रसिद्धीपत्रक",
    back: "मुख्यपृष्ठावर जा",
    srno: "अ. क्र.",
    date: "दिनांक",
    gazetteTitle: "प्रसिद्धीपत्रक शीर्षक",
    view: "पहा",
    download: "डाउनलोड",
    none: "सध्या कोणतेही  उपलब्ध नाही.",
    loading: "लोड होत आहे…",
    error: "प्रसिद्धीपत्रक लोड करता आले नाही.",
  },
};
...
export default function GovernmentPublications() {
  ...
  const { data: gazettes = [], isLoading, error } = useQuery({
    queryKey: ["public", "gazettes"],
    queryFn: async (): Promise<Gazette[]> => {
      const { data, error } = await supabase
        .from("gazettes")
        .select("id, title_en, title_mr, pdf_url, published_date, file_size_kb")
        ...
        <CardContent className="p-6 overflow-x-auto">
          {isLoading ? (
            ...
            <table className="w-full text-left border-collapse border border-gray-300 dark:border-gray-700">
              ...
              <tbody>
                {gazettes.map((g, index) => (
                  <tr
                    key={g.id}
                    className="hover:bg-blue-50 dark:hover:bg-blue-900/20 text-center transition-all"
                  >
                    <td className="border border-gray-300 px-3 py-2">{index + 1}</td>
                    <td className="border border-gray-300 px-3 py-2">{formatDate(g.published_date, lang)}</td>
                    <td className="border border-gray-300 px-3 py-2 text-left">
                      {lang === "mr" ? g.title_mr : g.title_en}
                    </td>
                    ...
                    <td className="border border-gray-300 px-3 py-2">
                      {g.pdf_url ? (
                        ...
                        >
                          ...
                          {g.file_size_kb ? ` (${formatFileSize(g.file_size_kb)})` : ""}
                        </a>
                      ) : (
                        <span className="text-muted-foreground">—</span>
                      )}
